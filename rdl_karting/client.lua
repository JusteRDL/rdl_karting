...
-- Menu --
local open = false
local MenuKarting = RageUI.CreateMenu("RDL DEV", "INTERACTION")
                ...
                RageUI.IsVisible(MenuKarting, function()
                    ...
                    RageUI.Button("Rendre le Kart", nil, {RightLabel = "~y~→"}, true, {
                        onSelected = function()
                            ...
                            if dist4 < 4 then
                                DeleteEntity(veh)
                                ESX.ShowAdvancedNotification("Karting", "~o~Karting", "Le véhicule a bien été ~o~rentrer~s~. Merci à vous !", "CHAR_DOM", 1)
                                RageUI.CloseAll()
                            end
                        end
                    })
                    ...
                    for k, v in pairs(Config.Kart) do
                        RageUI.Button(""..v.Label, nil, {RightLabel = "~c~"..v.Price.."$"}, true, {
                            onSelected = function()
                                TriggerServerEvent('RDLkarting:SpawnKart', v.Label, v.Value, v.Price)
                                RageUI.CloseAll()
                            end
                        })
                    end
...
-- Spawn du véhicule --
RegisterNetEvent('RDLkarting:spawnCar')
AddEventHandler('RDLkarting:spawnCar', function(car)
    ...
    local vehicle = CreateVehicle(car, -1023.81, -3489.56, 14.14, 59.41, true, false)
    ESX.ShowAdvancedNotification("Karting", "", "Votre ~o~Kart ~s~à bien été sortie ! ~o~Bonne course ~s~!", "CHAR_DOM", 1)
end)
...
zone = vector3(-1012.27, -3488.5, 13.14)
Heading = 155.1
Ped = nil
